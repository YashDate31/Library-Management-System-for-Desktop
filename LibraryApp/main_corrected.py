...
class LibraryManagementApp:
    def __init__(self, root):
        ...
        self.root.configure(bg='#2b3e56')  # Dark blue background like in image

        # Show admin login first
        self.show_login()

    def show_login(self):
        # Create login screen with dark blue background matching the image
        self.login_frame = tk.Frame(self.root, bg='#2b3e56')
        self.login_frame.pack(expand=True, fill=tk.BOTH)
        
        # Create central login card
        login_card = tk.Frame(self.login_frame, bg='#3a5373', relief=tk.FLAT, bd=0)
        login_card.place(relx=0.5, rely=0.5, anchor='center', width=400, height=350)
        
        # Add lock icon and title
        title_frame = tk.Frame(login_card, bg='#3a5373')
        title_frame.pack(pady=(30, 10))
        
        # Lock icon with "Admin Login" text like in the image
        tk.Label(
            title_frame, 
            text="🔐 Admin Login", 
        ...
        # Login button with cyan color like in image
        login_btn = tk.Button(
            form, 
            text="🔓 Login", 
