...
class LibraryManagementApp:
    def __init__(self, root):
        self.root = root
        self.root.title("📚 Library Management System v2.4.0")
    ...
    def create_navigation_bar(self):
        ...
        nav_buttons = [
            ("🏠 Dashboard", self.show_dashboard, '#ff6b6b'),
            ("👥 Students", self.show_students, '#4ecdc4'),
            ("📚 Books", self.show_books, '#45b7d1'),
            ("📋 Transactions", self.show_transactions, '#f9ca24')
        ]
    ...
    def show_book_history(self):
        ...
        history_win.title("📚 Book Borrow/Return History")
    ...
    def show_student_history(self):
        ...
        history_win = tk.Toplevel(self.root)
        history_win.title("👥 Student Borrow/Return History")
        history_win.geometry("1000x650")
        history_win.configure(bg='#1a1a2e')
        ...
        title_label = tk.Label(
            history_win,
            text="👥 Student Transaction History",
            font=('Segoe UI', 18, 'bold'),
            bg='#1a1a2e',
            fg='#ccd6f6'
        )
    ...
    def create_dashboard_view(self):
        ...
        stats = [
            ("Total Students", len(students), '#ff6b6b', '👥'),
            ("Total Books", len(books), '#4ecdc4', '📚'),
            ("Books Borrowed", len(borrowed), '#ffa726', '📤'),
            ("Available Books", sum(book[6] for book in books) if books else 0, '#26de81', '✅')
        ]
    ...
    def show_add_student_dialog(self):
        ...
        # Form fields
        tk.Label(dialog, text="👤 Add New Student", font=('Arial', 16, 'bold'), bg='#fff5f5', fg='#2c3e50').pack(pady=15)
        ...
        save_btn = tk.Button(btn_frame, text="💾 Save Student", command=save_student, bg='#28a745', fg='white', width=15, height=2, font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, cursor='hand2')
        save_btn.pack(side=tk.LEFT, padx=10)
        
        cancel_btn = tk.Button(btn_frame, text="❌ Cancel", command=dialog.destroy, bg='#dc3545', fg='white', width=15, height=2, font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, cursor='hand2')
    ...
    def show_add_book_dialog(self):
        ...
        # Form fields
        tk.Label(dialog, text="📚 Add New Book", font=('Arial', 16, 'bold'), bg='#fff5f5', fg='#2c3e50').pack(pady=15)
        ...
        save_btn = tk.Button(btn_frame, text="💾 Save Book", command=save_book, bg='#17a2b8', fg='white', width=15, height=2, font=('Segoe UI', 11, 'bold'), relief=tk.FLAT, cursor='hand2')
        save_btn.pack(side=tk.LEFT, padx=10)
    ...
    def select_enrollment_suggestion(self, event):
        ...
        if selection:
            ...
            if student:
                self.borrow_student_details.config(text=f"✓ Student: {student[2]} | Department: {student[5]} | Year: {student[6]}")
            else:
    ...
    def select_book_suggestion(self, event):
        ...
        if selection:
            ...
            if book:
                self.borrow_book_details.config(text=f"✓ Book: {book[2]} | Author: {book[3]} | Available: {book[6]} copies")
            else:
    ...
    def select_return_enrollment_suggestion(self, event):
        ...
        if selection:
            ...
            if borrowed_books:
                ...
                self.return_details.config(text=f"✓ Borrowed Books: {book_list}")
            else:
    ...
    def select_return_book_suggestion(self, event):
        ...
        if selection:
            ...
            if borrowed_books:
                ...
                self.return_details.config(text=f"✓ Returning: {book[5]} | Borrowed on: {book[7]} | Due: {book[8]}")
            else:
    ...
    def import_students_excel(self):
        ...
        try:
            ...
            instruction_msg = (
                ...
                "• enrollment_no (or Enrollment No) - REQUIRED ✓\n"
            ...
            if error_count > 0:
                result_msg += f"⚠️ Errors encountered: {error_count}\n\n"
                if error_details:
    ...
    def import_books_excel(self):
        ...
        try:
            ...
            instruction_msg = (
                "📋 Excel Import Instructions:\n\n"
                "MANDATORY COLUMNS (required):\n"
                "• book_id (or Book ID) - REQUIRED ✓\n"
                "• title (or Title) - REQUIRED ✓\n\n"
                "OPTIONAL COLUMNS:\n"
                "• author (or Author)\n"
                "• isbn (or ISBN)\n"
                "• category (or Category)\n"
                "• total_copies (or Total Copies) - defaults to 1\n\n"
                "The first row should contain column headers.\n"
                "Do you want to continue?"
            )
            ...
            # Show results
            result_msg = f"✅ Import completed!\n\n"
            result_msg += f"📊 Successfully imported: {success_count} books\n"
